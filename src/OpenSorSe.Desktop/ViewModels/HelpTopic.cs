...
public static class HelpCatalog
{
    ...
    [
        ...
        Topic(HelpTopicId.Privacy, "Privacy", "Explains what local indexing and optional AI can retain or transmit.",
            ...
            ["Remember that scanning, indexing, extraction, and derived Search storage are local by default.", "Inspect stored data for a file or source.", "Exclude sensitive folders or types when needed.", "Treat every non-loopback AI endpoint as remote.", "If a future OmniExplorer is explicitly authorized, review the selected indexed-source scope and end the session when finished.", "Review diagnostic exports before sharing."],
            ...
            "OmniSorSe has no telemetry or cloud-indexing dependency. A configured remote Ollama-compatible endpoint is outside that local boundary and may receive only the information supplied to an explicitly enabled AI request. Explorer Protocol v1 has no network listener, is dormant without an explicit session, omits raw paths by default, and exposes no file mutations; OmniExplorer itself is not included in v2.4.", [HelpTopicId.Settings, HelpTopicId.AiSetup, HelpTopicId.MediaIntelligence, HelpTopicId.ContentIntelligence, HelpTopicId.Diagnostics]),
