...
| v2.12 candidate | `v2.12-trusted-relationships-context` | 2026-08-20 | Trusted Relationships & Context. | Stronger bounded relationship evidence, reversible pair authority, direct Related Files, relationship-only reanalysis, and format-2 authored-state backup. | 1,861 integrated tree | Candidate and engineering-system commits are ancestors of published GitHub `main`; the exact candidate branch ref is also remote. The initial published-main hosted run exposed matching macOS ARM/Intel test-portability failures; correction-tip and exact-main hosted validation subsequently passed all four hosts. No release tag/package is claimed. |
...
- v2.5-v2.12 were developed as one linear local candidate chain after v2.4.0.
  ...
  complete chain became an ancestor of `main` through merge `be536a0`. A normal
  atomic push then published `main` at `cc6c331` and created every v2.5-v2.12
  remote branch ref at its exact local tip; no force or rewrite was used.
- A disposable clean clone of the remote default `main` checked out `cc6c331`
  and independently passed no-cache restore, a zero-warning Release build, and
  1,861/1,861 Release tests. Source publication did not create v2.5-v2.12 tags,
  packages, or GitHub Releases.
