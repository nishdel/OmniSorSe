...
This report separates locally executed evidence from hosted-runner
configuration. A workflow definition is not presented as proof that GitHub has
executed it.
...
| Debug tests with `--no-build --no-restore` | 890 passed; zero failed; zero skipped/not executed. |
| Release build with `--no-restore` | Passed; zero warnings and zero errors. |
| Release tests with `--no-build --no-restore` | 890 passed; zero failed; zero skipped/not executed. |
...
produced six result files with `total=890`, `executed=890`, `passed=890`,
...
documentation/dependency policy. Hosted execution is not claimed until GitHub
