...
## v1.5 filename portability

Recipes now declare portable, Windows-compatible, or current-platform filename
semantics. Missing persisted values default to the prior conservative portable
behavior. Import/export preserves the policy and never silently rewrites it.
All modes retain traversal, root, collision, non-overwrite, review, and
validation checks. See [Workflow Portability](../../WORKFLOW_PORTABILITY_v1.5.md).
