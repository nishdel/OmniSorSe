...
## v1.5 Linux preview behavior

On Linux, the watcher adapter is backed by inotify through
`FileSystemWatcher`. Descriptor limits, queue overflow, duplicate/coalesced
events, root/mount loss, and temporary replacement saves are explicit
limitations. Events remain hints and bounded startup/manual/periodic/overflow
reconciliation reads actual state. Symbolic links are excluded and watcher
processing cannot escape the configured root or invoke mutation. See
