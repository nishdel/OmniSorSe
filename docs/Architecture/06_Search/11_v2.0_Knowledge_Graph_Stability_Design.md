...
**Status:** Accepted stability design implemented as a candidate on
`v2.0-knowledge-graph`; release-candidate and manual validation remain open.

**Implementation status:** This document remains the design authority, while
source and tests define the implementation. The candidate adds isolated graph
and decision sidecars, provider-neutral services, projection lifecycle, bounded
UI, and optional Search context. See the
[implementation guide](../../KNOWLEDGE_GRAPH_v2.0.md). It is not a merge,
package, tag, publication, RC, or manual-validation claim.

## Objective

The v2.0 candidate projects indexed files, evidence-backed relationships, Smart
...
v2.0 still requires a release-candidate stabilization phase after feature freeze.
...
The implementation-candidate reports are the
[implementation report](../../V2.0_IMPLEMENTATION_REPORT.md), pending
[validation report](../../V2.0_VALIDATION_REPORT.md), and
[RC stabilization plan](../../V2.0_RC_STABILIZATION_PLAN.md). Their open
gates do not weaken this design's release blockers.
