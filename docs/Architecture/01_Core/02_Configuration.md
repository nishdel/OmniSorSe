...
OpenSorSe uses a validated JSON settings file of at most 1 MiB in local application data. Implemented groups cover logging, feature visibility, diagnostics, catalog, optional AI, bounded content/OCR, compatible `SemanticSearch` document/result limits, and `DeepIndexing` policy. Deep-index settings cover Basic/Standard/Deep, resource mode/concurrency, quota, text/OCR/chunk bounds, deleted/failure retention, retries, idle/power/battery preferences, OCR/AI/archive/generated/binary policy, and an optional time window. Missing older properties receive safe defaults and every settings-copy/migration path preserves the new group. Content, Search, history, AI decisions, catalog, and saved searches use separate stores. The only environment override is `OPENSORSE_LOGGING__MINIMUMLEVEL`.
