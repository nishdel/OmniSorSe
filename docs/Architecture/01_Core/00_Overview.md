...
`OpenSorSe.Core` supplies the current domain models and shared abstractions used by the .NET 10 application. `OpenSorSe.Application` and the Desktop composition root provide the current lifecycle, dependency-injection, settings, diagnostics, and operation-history integration around those models.
