...
v1.1 preserves the local-first, suggestion-only AI boundary. Accepted rename/folder and deterministic organization suggestions become a non-mutating Change Plan. Only user-approved, validated actions can enter the dedicated execution service after final confirmation. Every attempt is durably journalled; successful rename/move/directory actions have conflict-aware inverse information. No specification authorizes autonomous AI filesystem control, permanent deletion, monitoring, plugins, cloud indexing, or unreviewed execution.
