...
v1.3 preserves the local-first, suggestion-only AI and v1.1 execution boundary. Reusable profiles resolve typed scan/analysis policy and recipes create deterministic proposals; neither grants approval. Watched roots automate detection, stability checking, incremental analysis, catalogue reconciliation, and optional suggestion creation. Suggestions remain non-mutating Change Plans; only user-approved, validated actions can enter the dedicated execution service after final confirmation. No specification authorizes autonomous AI filesystem control, permanent deletion, plugins, cloud indexing, or unreviewed execution.

The current release is `v1.3`, **Workflow Profiles and Recipe Library**, on `v1.3-workflow-profiles`. Release branches follow `v<version>-<primary-feature>`, for example `v1.1-safe-file-operations`, `v1.2-watched-folders`, and `v1.3-workflow-profiles`.
