...
**Status:** required and not started

The implementation branch is not a release candidate merely because automated
implementation validation succeeds. The following stabilization phase must run
from one immutable candidate commit. Results belong in an RC evidence record;
this plan contains no completed claims.
...
Until every applicable item is evidenced, v2.0 remains an unmerged
